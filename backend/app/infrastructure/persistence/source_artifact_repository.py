    ...
    def save(self, artifact: SourceArtifactMetadata) -> None:
        with self.database.engine.begin() as connection:
            connection.execute(
                ...
                {
                    "i": artifact.artifact_id,
                    "p": artifact.provider_id,
                    "r": artifact.retrieved_at,
                    "h": artifact.content_hash,
                    "c": artifact.content_type,
                    "s": artifact.size_bytes,
                    "o": artifact.object_reference,
                    "is": artifact.ingestion_status,
    ...
    def get(self, artifact_id: str) -> SourceArtifactMetadata | None:
        with self.database.engine.connect() as connection:
            row = connection.execute(
                text("SELECT * FROM source_artifacts WHERE artifact_id=:id"),
                {"id": artifact_id},
            ).mappings().first()
        if row is None:
            return None
